...
bool Log::SendFromPanel() {
    constexpr const char* kLabels[] = {"TRACE", "DEBUG", "INFO", "WARN", "ERROR", "FATAL"};
...
void Log::Debug(DebugUI* _debug) {
    ...
    _debug->RegisterCommand("Logger",
        [_debug] {
            ...
            constexpr const char* kLabels[6] = {"TRACE", "DEBUG", "INFO", "WARN", "ERROR", "FATAL"};
            ...
            for (int i = 0; i < 6; i++) {
                ...
                ImGui::Checkbox(kLabels[i], &filters[i]);
                ImGui::PopStyleColor();
            }
