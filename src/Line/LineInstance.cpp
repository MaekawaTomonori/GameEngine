...
void LineInstance::ExecuteDraw() const {
...
void LineInstance::AddLine(const Vector3& _start, const Vector3& _end) {
...
void LineInstance::Clear() {
    positions_.clear();
}

void LineInstance::SetColor(const Vector4& _color) const {
    if (!materialData_){
...
void LineInstance::SetName(const std::string& _name) {
    name_ = _name;
}

void LineInstance::CreateVertexBuffer() {
...
void LineInstance::CreateMaterialBuffer() {
    materialResource_ = adapter_->CreateBufferResource(sizeof(Material));
    materialResource_->Get()->Map(0, nullptr, reinterpret_cast<void**>(&materialData_));
}

void LineInstance::CreateTransformationBuffer() {
